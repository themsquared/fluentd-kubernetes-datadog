...
RUN [ -f /bin/entrypoint.sh ] && /bin/entrypoint.sh echo || : && \
    ...
    gem install fluent-plugin-systemd fluent-plugin-record-reformer fluent-plugin-kubernetes_metadata_filter fluent-plugin-concat fluent-plugin-record-modifier fluent-plugin-datadog && \
